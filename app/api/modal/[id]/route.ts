...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase
    .from("modals")
    .select(
      "title, body, button, show_confirmation, button_color, button_link, is_exit_intent, background_color, text_color, button_text_color, is_modal_centered"
    )
    .eq("id", params.id)
    .single();
  ...
  (function() {
    ...
    var modalHtml = '<div id="custom-modal" style="position:fixed;z-index:1000;left:0;top:0;width:100%;height:100%;background-color:rgba(0,0,0,0.5);display:none;">' +
      ...
      '<div style="color:' + (modal.text_color || '#000000') + ';' + (modal.is_modal_centered ? 'text-align:center;' : '') + '">' + 
      '<h2 style="margin-bottom:15px;font-weight:bold;">' + modal.title + '</h2>' +
      '<p>' + modal.body + '</p>' +
      '</div>' +
     '<div style="' + (modal.is_modal_centered ? 'text-align:center;' : 'text-align:right;') + 'margin-top:35px;">';
